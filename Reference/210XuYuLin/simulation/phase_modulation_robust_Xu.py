...
def robust_sequence(nseg, tau, mu, omega_k, g_ik, nk, x0='search',
    ...
    return(infid(phase_full(sol.x), M_ij * rabi**2, gamma_ij * rabi**2), 
           rabi, phase_full(sol.x),sol.x, gamma_ij, M_ij)
...
def robustness_detuning(nseg, tau, mu, omega_k, detuning, g_ik, nk, x0=None,
    ...
    t_list = np.linspace(0, tau, nseg + 1)
    alpha = alpha_seg(t_list[:-1], t_list[1:], mu, omega_k + detuning, ramp_tau, ramp)
    alpha1_nk = g_ik[0, :].reshape((1, -1)) * alpha
    alpha2_nk = g_ik[1, :].reshape((1, -1)) * alpha
    beta_k = 2 * (2 * nk.reshape((1, -1)) + 1)
    M_ij = (alpha1_nk.conj() * beta_k) @ alpha1_nk.T \
    ...
    theta = phi_seg(t_list[:-1], t_list[1:], mu, omega_k + detuning, ramp_tau, ramp)
    gamma_ij = np.sum((g_ik[0, :] * g_ik[1, :]).reshape((1, 1, -1)) * theta,
                      axis=2)
    
    gamma_ij = (gamma_ij - gamma_ij.T.conj()) / 2j
    
    rabi = getRabi(x0, gamma_ij)
    err = infid(x0, M_ij * rabi**2, gamma_ij * rabi**2)
...
if __name__ == "__main__":
    ...
    mu = 2 * np.pi * 1.5871
    # mu = 2 * np.pi * 1.6215
    # omega_k = 2 * np.pi * np.array([1.5705,1.6015])
    omega_k = 2 * np.pi * np.array([1.4694,1.5265,1.5711,1.6021])
    ...
    # b_ik = np.array([[-0.70710678, 0.70710678], [0.70710678, 0.70710678]])
    g_ik = eta_k.reshape((1, -1)) * b_ik
    nk = np.ones(4) * 0
    
    # t1 = np.array([0, 10, 20, 30])
    # # t2 = np.array([10, 20, 30, 40])
    # x0 = 2*np.pi*np.array([0.416, 0.648,  0.435,  0.593, 0.663, 0.887, 0.97, 0, 
    #                         -0.97, -0.887, -0.663, -0.593, -0.435, -0.648, -0.416])
    # x0 = np.array([ 6.91781885,  4.60760163,  3.1800696 ,  6.92286933,  4.89669197,
    #             3.26776697,  1.72750746,  0.        , -1.72750746, -3.26776697,
    #             -4.89669197, -6.92286933, -3.1800696 , -4.60760163, -6.91781885])
    
    x0 = np.array([ -2.27265899,  -2.41005601,   4.78883342,  10.71230074,
             4.5173664 ,   5.09646242,  12.07551613,   0.        ,
           -12.07551613,  -5.09646242,  -4.5173664 , -10.71230074,
            -4.78883342,   2.41005601,   2.27265899])
    # x0 = np.array([-1.65164859,  4.56662095, -2.23102146,  4.04792468,  3.55466796,
    #         2.50602205,  2.5172879 ,  1.65135314,  0.88612583,  0.61333892,
    #        -0.61333892, -0.88612583, -1.65135314, -2.5172879 , -2.50602205,
    #        -3.55466796, -4.04792468,  2.23102146, -4.56662095,  1.65164859])
    # print(phi_seg_diag(t1, t2, mu, omega_k, tau=0, ramp='sin'))
    # print(alpha_seg(t1, t2, mu, omega_k, tau=0, ramp='sin'))
    infd, rabi, phaseseq, x, M_ij0, gamma_ij0 = robust_sequence(nseg, tau, mu, omega_k, g_ik, nk, x0='random',
                                              display=False, regularization=20, 
    ...
    for detuning in detuning_list:
        # detuning = 0
        err, rabi0 = robustness_detuning(nseg, tau, mu, omega_k, detuning, g_ik, nk, x0=phaseseq, ramp_tau=ramp_tau, ramp='sin')
        err_rabi = robustness_detuning_rabifix(nseg, tau, mu, rabi, omega_k, detuning, g_ik, nk, x0=phaseseq, ramp_tau=ramp_tau, ramp='sin')
        err_list.append(err)
    ...
    err_phase_robust = []
    for i in range(100):
        dphase_ran = np.random.randn(nseg)*0.01*np.pi*2
        phaseseq_exp = phaseseq + dphase_ran
        err_rabi = robustness_detuning_rabifix(nseg, tau, mu, rabi, omega_k, 0, g_ik, nk, x0=phaseseq_exp, ramp_tau=ramp_tau, ramp='sin')
    ...
    x = (x/2/np.pi)%1
    if nseg % 2==0:
        seq_2pi = np.concatenate((x, -x[::-1]))
    else:
        seq_2pi = np.concatenate((x, [0], -x[::-1]))
    
    ax2 = plt.subplot(2, 2, 2)
    plt.bar(range(len(phaseseq)), seq_2pi)
    plt.title("phaseseq, rabi rate=%.3f"%rabi)
