...
def robust_sequence(nseg, tau, mu, omega_k, g_ik, nk, x0='search',
                    ...
                    ramp_tau=0, ramp='linear', sep=0, g_phase=np.pi/4, return_sign=False):
    # g_ik = b_ik * eta_k for two target ions
    # n_k thermal phonon number for each mode
    # sep: separation between segments due to switching
    # g_phase: the geometric phase accumulated at the end of the gate seq
    tau_seg = (tau - sep * (nseg - 1)) / nseg
    assert tau_seg > 0
    t1_list = np.linspace(0, tau - tau_seg, nseg)
    t2_list = np.linspace(tau_seg, tau, nseg)

    alpha = alpha_seg(t1_list, t2_list, mu, omega_k, ramp_tau, ramp)
    ...
    theta = phi_seg(t1_list, t2_list, mu, omega_k, ramp_tau, ramp)
    ...
    rabi = getRabi(phase_full(sol.x), gamma_ij, g_phase=g_phase, return_sign=return_sign)
    return(infid(phase_full(sol.x), M_ij * rabi**2, gamma_ij * rabi**2, g_phase=g_phase), 
           rabi/2/np.pi, sol)


def robustness_detuning(nseg, tau, mu, omega_k, detuning, g_ik, nk, x0=None,
                        ramp_tau=0, ramp='linear', sep=0, g_phase=np.pi/4, return_sign=False):
    ...
    alpha = alpha_seg(t1_list, t2_list, mu, omega_k + detuning, ramp_tau, ramp)
    alpha1_nk = g_ik[0, :].reshape((1, -1)) * alpha
    alpha2_nk = g_ik[1, :].reshape((1, -1)) * alpha
    beta_k = 2 * (2 * nk.reshape((1, -1)) + 1)
    M_ij = (alpha1_nk.conj() * beta_k) @ alpha1_nk.T \
    ...
    theta = phi_seg(t1_list, t2_list, mu, omega_k + detuning, ramp_tau, ramp)
    gamma_ij = np.sum((g_ik[0, :] * g_ik[1, :]).reshape((1, 1, -1)) * theta,
                      axis=2)
    
    gamma_ij = (gamma_ij - gamma_ij.T.conj()) / 2j
    
    rabi = getRabi(x0, gamma_ij, g_phase=g_phase, return_sign=return_sign)
    err = infid(x0, M_ij * rabi**2, gamma_ij * rabi**2, g_phase=g_phase)
    
    return err, rabi/2/np.pi



def robustness_detuning_rabifix(nseg, tau, mu, rabi, omega_k, detuning, g_ik, nk, x0=None,
                                ramp_tau=0, ramp='linear', sep=0, g_phase=np.pi/4):
...
def obj_fun(x, M, G, gamma, regularization=0, sign=None):
    '''
    sign: the sign of the geometric phase
    '''
...
def getRabi(x, gamma, g_phase=np.pi/4, return_sign=False):
    phi_ij = x.reshape((-1, 1)) - x.reshape((1, -1))
    cos_ij = np.cos(phi_ij)
    sin_ij = np.sin(phi_ij)
    theta = np.sum(np.real(gamma) * cos_ij - np.imag(gamma) * sin_ij)
    scale = g_phase / np.abs(theta)
    if not return_sign:
        return(np.sqrt(scale))
    else:
        return(np.sign(theta)*np.sqrt(scale))


def infid(x, M, gamma, g_phase=np.pi/4):
...
def plot_robustness(nseg, tau, mu, omega_k, rabi, detuning_list, g_ik, nk, x0=None, ramp_tau=2, ramp='sin', sep=0, g_phase=np.pi/4, return_sign=False):
    ...
    phaseseq = x0*2*np.pi
    infd = robustness_detuning_rabifix(nseg, tau, mu, rabi*2*np.pi, omega_k, 0, g_ik, nk, x0=phaseseq, ramp_tau=ramp_tau, ramp='sin', sep=0, g_phase=g_phase)
    
    for detuning in detuning_list:
        # detuning = 0
        err, rabi0 = robustness_detuning(nseg, tau, mu, omega_k, detuning, g_ik, nk, x0=phaseseq, ramp_tau=ramp_tau, ramp='sin', sep=0, g_phase=g_phase, return_sign=return_sign)
        err_rabi = robustness_detuning_rabifix(nseg, tau, mu, rabi*2*np.pi, omega_k, detuning, g_ik, nk, x0=phaseseq, ramp_tau=ramp_tau, ramp='sin', sep=0, g_phase=g_phase)
        err_list.append(err)
        err_rabifix.append(err_rabi)
        rabi_list.append(rabi0)
    
    plt.figure(figsize=(15, 10))
    plt.subplot(2, 2, 1)
    plt.plot(detuning_list/2/np.pi, err_list, 'o')
    plt.xlabel("detuning/MHz")
    plt.ylabel("error")
    plt.title("Error without rabi rate fixed")
    
    plt.subplot(2, 2, 2)
    plt.bar(range(len(phaseseq)), x0)
    plt.title("phaseseq, rabi rate=%.3f MHz"%rabi)
    
    plt.subplot(2, 2, 3)
    plt.plot(detuning_list, rabi_list, 'o')
    plt.xlabel("detuning/MHz")
    plt.ylabel("rabi/MHz")
    
    plt.subplot(2, 2, 4)
    plt.plot(detuning_list/2/np.pi, err_rabifix, 'o')
    plt.xlabel("detuning/MHz")
    plt.ylabel("error")
    plt.title("Error with rabi rate fixed, error=%.4f"%infd)
...
if __name__ == "__main__":
    ...
    mu = 2 * np.pi * (1.3812+0.015)
    omega_k = 2 * np.pi * np.array([1.3812, 1.3474, 1.2984])
    eta_k = 2 * 2 * np.pi / 411e-9 * np.sqrt(6.62607e-34 / 2 / np.pi / 2 / 171 / 1.66054e-27 / omega_k / 1e6)*np.cos(np.pi/4)

    b_ik = np.array([[-5.77350269e-01, -2.59514632e-15,  8.16496581e-01],
                     [-5.77350269e-01, -7.07106781e-01, -4.08248290e-01]])
    # b_ik = np.array([[ 0.21661412, -0.5       , -0.67311093,  0.5       ],
    #                  [-0.21661412, -0.5       ,  0.67311093,  0.5       ]])
    g_ik = eta_k.reshape((1, -1)) * b_ik
    nk = np.ones(len(omega_k)) * 0.1
    
    
    # x0 = 2*np.pi*np.array([ 0.412,  0.847,  0.729,  0.371,
    #         0.782,  0.818,  0.79,  0.923,
    #         0.964,  0.001, -0.001, -0.964,
    #        -0.923, -0.79, -0.818, -0.782,
    #        -0.371, -0.729, -0.847, -0.412])
    
    # x0 = 2*np.pi*np.array([ 0.63829504,  0.61642765,  0.76216651,  0.70491567,  0.7189611 ,
    #     0.81112719,  0.92187802,  0.        , -0.92187802, -0.81112719,
    #    -0.7189611 , -0.70491567, -0.76216651, -0.61642765, -0.63829504])
    ...
    # Apply single frequency oscillation
        omega_new, g_new = single_frequency_oscillation(omega_k, g_ik, nu, dphi, cut)
    ...
    infd, rabi, sol = robust_sequence(nseg, tau, mu, omega_new, g_new, n_new, x0="search",
                                              display=False, regularization=10, 
                                              ramp_tau=ramp_tau, ramp='sin', sep=tau_sep, g_phase=g_phase,return_sign=True)
    ...
    phaseseq = seq_2pi
    # phaseseq = np.round(x0/2/np.pi, 4)
    # rabi = 0.102
    
    #analyze the robustness of the PM sequence against the detuning shift
    detuning_list = np.linspace(-0.003, 0.003, 20)*2*np.pi
    
    plot_robustness(nseg, tau, mu, omega_k, rabi, detuning_list, g_ik, nk, x0=phaseseq, ramp_tau=ramp_tau, ramp='sin', sep=tau_sep, g_phase=g_phase,return_sign=True)
    ...
    # for detuning in detuning_list:
    #     # detuning = 0
    #     err, rabi0 = robustness_detuning(nseg, tau, mu, omega_k, detuning, g_ik, nk, x0=phaseseq, ramp_tau=ramp_tau, ramp='sin', sep=0)
    #     err_rabi = robustness_detuning_rabifix(nseg, tau, mu, rabi*2*np.pi, omega_k, detuning, g_ik, nk, x0=phaseseq, ramp_tau=ramp_tau, ramp='sin', sep=0)
    #     err_list.append(err)
    #     err_rabifix.append(err_rabi)
    #     rabi_list.append(rabi0)
    #     # if detuning == 0:
    ...
    # err_phase_robust = []
    # for i in range(100):
    #     dphase_ran = np.random.uniform(-1, 1, nseg)*0.01*np.pi*2
    #     phaseseq_exp = phaseseq + dphase_ran
    #     err_rabi = robustness_detuning_rabifix(nseg, tau, mu, rabi*2*np.pi, omega_k, 0, g_ik, nk, x0=phaseseq_exp, ramp_tau=ramp_tau, ramp='sin', sep=0)
    #     err_phase_robust.append(err_rabi)
    ...
    # plt.bar(range(len(phaseseq)), seq_2pi)
    # plt.title("phaseseq, rabi rate=%.3f MHz"%rabi)
    ...
    # plt.figure()
    # plt.hist(err_phase_robust, bins=10)
    # plt.title("Phase setting robustness, mean=%.3f"%np.mean(err_phase_robust))
